...
namespace RMC.BlockWorld.Standard
{
    public static class CustomColorUtility
    {
        ...
        public static void SetColorStandardRP(Material material, Color toColor)
        {
            material.SetColor("_Color", toColor);
        }
        
        public static Color GetColorStandardRP(Material material)
        {
            return material.GetColor("_Color");
        }
        
        public static void SetColorURP(Material material, Color toColor)
        {
            material.SetColor("_BaseColor", toColor);
        }
        
        public static Color GetColorURP(Material material)
        {
        ...
        public static async Task SetColorAsync(Material material, Color toColor, float durationInSeconds)
        {
            ...
            Color fromColor = GetColorURP(material);

            while (Time.time < endTime)
            {
                float elapsed = Time.time - startTime;
                float percent = Mathf.Clamp01(elapsed / durationInSeconds);
                SetColorURP(material, Color.Lerp(fromColor, toColor, percent));

                await Task.Yield();
            }
            
            // Ensure the final color is set to the target color
            SetColorURP(material, toColor);
        }
    }
}
